...
export const SellingCompaniesLogin = (props) => {
  const [companyName, setCompanyName] = useState('');
  const [password, setPassword] = useState('');
 
 const handleSubmit = (e) => {
  e.preventDefault();
  axios.post('http://localhost:8080/demo-1.0-SNAPSHOT/api/sellingcompany/login', {
  ...
  .then(response => {
    if (JSON.stringify(response.data).includes('Login Successfully')) {
      ...
      const selling_company_id = response.data.selling_company_id;
      localStorage.setItem('selling_company_id', selling_company_id);
      props.onFormSwitch('sellingCompaniesPage');
    } else {
      alert('Login Failed');
    }
  })
